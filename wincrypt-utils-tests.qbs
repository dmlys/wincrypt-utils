	...
	cpp.dynamicLibraries:
	{
		if (qbs.toolchain.contains("gcc") || qbs.toolchain.contains("clang"))
		{
			...
			if (qbs.toolchain.contains("mingw"))
				libs = libs.concat(["ws2_32", "ncrypt", "crypt32", "ssp"]) // ssp is for mingw(gcc) stack protector, _FORTIFY_SOURCE stuff
		...
		if (qbs.toolchain.contains("msvc"))
		{
			...
			libs = libs.concat(["ws2_32", "crypt32", "ncrypt", "user32", "advapi32"])
			return libs
		}
